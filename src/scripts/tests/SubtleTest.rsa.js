...
function importRsaKeyTest(keyJsonText, algorithmName, isPublic, isSync) {
    ...
    keyOp.oncomplete = function (e) {
        ...
        keyOp1.oncomplete = function (e1) {
            ...
            compareKeys(shared.getArrayResult(e1.target.result), expectedKeyText);
        };

        keyOp1.onerror = shared.error();
    };

    keyOp.onerror = shared.error();
}
...
function rsaDecrypt(privateKey, algorithmName, encryptedBytes, sync, process, expectedResults) {
    ...
    importRsaKey(privateKey, algorithmName, function (key) {
        ...
        if (process) {
            ...
            cryptoOp = subtle.decrypt(algObj, key, shared.toSupportedArray(encryptedBytes));
        }
...
function rsaSignComplete(context) {
    return function (e) {
        var signature = shared.toSupportedArray(shared.getArrayResult(e.target.result));
