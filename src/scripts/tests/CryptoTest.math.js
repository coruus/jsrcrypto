...
cryptoMathVectorTest.division = function (lines) {
    ...
    for (var i = 2; i < lines.length; i += 4) {
        ...
        // Verify the results
        if (!cryptoMath.sequenceEqual(actualQuotient, cDigits)) {
            ok(false, "Division vector test failed - quotient incorrect");
        }

        if (!cryptoMath.sequenceEqual(actualRemainder, dDigits)) {
...
// NAF Generation Vector Test
cryptoMathVectorTest.nafgen = function (lines) {

    // Loop through the test cases
    for (var i = 2; i < lines.length; i += 4) {
        var aBytes = cryptoMathVectorTest.readByteArray(lines[i]);
        var windowSize = parseInt(lines[i + 1]);
        var numDigitsExpected = parseInt(lines[i + 2]);
        var nafExpected = cryptoMathVectorTest.readIntArray(lines[i + 3]);

        // create digit array from the operand bytes
        var aDigits = cryptoMath.bytesToDigits(aBytes);

        // create working storage
        var nafActual = createArray(aDigits.length * cryptoMath.DIGIT_BITS + 1);
        var tmpStorage = createArray(aDigits.length);

        // perform the operation
        var numDigitsActual = cryptoMath.computeNAF(aDigits, windowSize, tmpStorage, nafActual);

        // check the answer
        if (numDigitsActual !== numDigitsExpected) {
            ok(false, "NAF generation vector test failed. NAF actual=" + numDigitsActual + ",expected="+numDigitsExpected);
        }

        if (cryptoMath.compareDigits(nafActual, nafExpected) !== 0) {
            ok(false, "NAF generation vector test failed.");
        }
    }
